...
try{
 ...
 INSERT INTO maintenance_movements(id,document_number,department,operational_area,movement_type,technician_user_id,agency_id,equipment_type,serial_number,quantity,created_by_user_id,created_by_name,qr_token,delivered_by_name,failure_cause,occurred_at) VALUES('11111111-1111-1111-1111-111111111111','DEMO-001','TECHNOLOGY','WAREHOUSE','ENTRY','tech',NULL,N'Equipo de prueba',N'SERIAL-DEMO',1,N'creator',N'Creador',N'REAL-MNT|ALM-20260910-000001-DEMO1',N'Remitente',N'Recepción de prueba',SYSUTCDATETIME());
 ...
 Check(await db.InstitutionalChange(requirementId,warehouse,"approve",new(Note:"Decisión antes de recibir"),ct)!=null,"Warehouse cannot authorize a requirement before receipt");
 Check(await db.InstitutionalChange(requirementId,warehouse,"receive",new(SealX:24,SealY:68),ct)==null&&(await db.InstitutionalDetail(requirementId,warehouse,ct))!.Status=="received","Warehouse receipt moves requirement to received table");
 Check(await db.InstitutionalChange(requirementId,warehouse,"approve",new(Note:"Equipo autorizado para entrega"),ct)==null&&(await db.InstitutionalDetail(requirementId,docOwner,ct))!.Status=="approved","Warehouse authorization after receipt is visible to requester");
