...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v270-forced-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$outputRoot = [System.IO.Path]::GetFullPath((Join-Path $outputsRoot "V270_FORZADO_DOBLE_RUTA"))
$packageRoot = Join-Path $outputRoot "paquete"
$destinationZip = Join-Path $outputRoot "FORZAR_VERSION_V270_DOBLE_RUTA.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V270_COMPROBANTE_PDF_RECIBIDO.txt"
$offlineMarker = Join-Path $projectRoot "deploy\app_offline.v270.nuevo"
...
Write-Host "1/5 Compilando la interfaz V270..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/5 Publicando el backend V270..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V270_COMPROBANTE_PDF_RECIBIDO.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V270_COMPROBANTE_PDF_RECIBIDO.txt") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $packageRoot "app_offline.v270.nuevo") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $secondApplicationRoot "app_offline.v270.nuevo") -Force
...
$required = @("RegistroAgencias.SqlServer.V270.exe","web.config","wwwroot/index.html","wwwroot/loto-real-logo-transparent.png","wwwroot/version-web.json")
...
Write-Host "Paquete V270 listo: $destinationZip" -ForegroundColor Green
