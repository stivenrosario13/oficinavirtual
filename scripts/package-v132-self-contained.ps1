...
$output=Join-Path $root "deploy\V132_AUTONOMO_SIN_RUNTIME"
$publish=Join-Path $root "artifacts\V132_SELF_CONTAINED"
...
$zip=Join-Path $output "01_SUBIR_EJECUTABLE_AUTONOMO_V132.zip"
if(Test-Path -LiteralPath $zip){Remove-Item -LiteralPath $zip -Force}
Add-Type -AssemblyName System.IO.Compression
Add-Type -AssemblyName System.IO.Compression.FileSystem
$archive=[System.IO.Compression.ZipFile]::Open($zip,[System.IO.Compression.ZipArchiveMode]::Create)
try{
 @("RegistroAgencias.SqlServer.V132.exe","RegistroAgencias.SqlServer.V132.staticwebassets.endpoints.json")|ForEach-Object{[System.IO.Compression.ZipFileExtensions]::CreateEntryFromFile($archive,(Join-Path $publish $_),$_,[System.IO.Compression.CompressionLevel]::Optimal)|Out-Null}
