...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v260-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V260_HISTORICOS_DISENO_UNIFICADO_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V260_HISTORICOS_DISENO_UNIFICADO.txt"
...
Write-Host "1/4 Compilando la interfaz V260..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V260..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V260_HISTORICOS_DISENO_UNIFICADO.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V260_HISTORICOS_DISENO_UNIFICADO.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V260 listo: $destinationZip" -ForegroundColor Green
