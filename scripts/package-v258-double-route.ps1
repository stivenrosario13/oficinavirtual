...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v258-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V258_PANEL_LOGIN_MENU_MOVIL_DOBLE_RUTA_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V258_PANEL_LOGIN_MENU_MOVIL_DOBLE_RUTA.txt"
...
Write-Host "1/4 Compilando la interfaz V258..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V258..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V258_PANEL_LOGIN_MENU_MOVIL_DOBLE_RUTA.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V258_PANEL_LOGIN_MENU_MOVIL_DOBLE_RUTA.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V258 listo: $destinationZip" -ForegroundColor Green
