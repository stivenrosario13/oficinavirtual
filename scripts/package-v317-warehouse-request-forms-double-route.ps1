...
$publish = Join-Path $root "outputs/_publish_v317_$stamp"
$package = Join-Path $root "outputs/_double_v317_$stamp"
$out = Join-Path $root "outputs/V317_FORMULARIOS_SOLICITUD_ALMACEN_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V317_FORMULARIOS_SOLICITUD_ALMACEN_$stamp.zip"
...
# Conserva exactamente el formato de doble ruta utilizado para las entregas
# recientes de Monster: publicación normal y segunda copia bajo wwwroot.
...
try {
  ...
  foreach ($prefix in @('','wwwroot/')) {
    foreach ($name in @(
      'RegistroAgencias.SqlServer.V317.exe',
      'RegistroAgencias.SqlServer.V317.dll',
      'RegistroAgencias.SqlServer.V317.deps.json',
      'RegistroAgencias.SqlServer.V317.runtimeconfig.json',
      'web.config',
      'wwwroot/index.html',
  ...
  if ($names | Where-Object { $_ -match 'V316|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\' }) {
  ...
  foreach ($entry in $archive.Entries | Where-Object FullName -match '(^|/)version-web\.json$') {
    ...
    if ($versionJson -notmatch '"release"\s*:\s*"V317"') { throw 'Versión web incorrecta.' }
  }
