...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v264-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V264_ACCESO_DIRECTO_ALMACEN_TALLER_COLORES_MOVIL_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V264_ACCESO_DIRECTO_COLORES_MOVIL.txt"
...
Write-Host "1/4 Compilando la interfaz V264..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V264..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V264_ACCESO_DIRECTO_COLORES_MOVIL.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V264_ACCESO_DIRECTO_COLORES_MOVIL.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V264 listo: $destinationZip" -ForegroundColor Green
