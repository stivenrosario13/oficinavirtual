...
$publish = Join-Path $root "outputs/_publish_v326_$stamp"
$package = Join-Path $root "outputs/_double_v326_$stamp"
$out = Join-Path $root "outputs/V326_ESTABILIDAD_SOPORTE_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V326_ESTABILIDAD_SOPORTE_$stamp.zip"
...
try {
  ...
  foreach($prefix in @('','wwwroot/')){foreach($name in @('RegistroAgencias.SqlServer.V326.exe','RegistroAgencias.SqlServer.V326.dll','RegistroAgencias.SqlServer.V326.deps.json','RegistroAgencias.SqlServer.V326.runtimeconfig.json','web.config','wwwroot/index.html','wwwroot/version-web.json','wwwroot/release-guard.js','Data/agency-locations-sonadora-20260907.json')){if(($prefix+$name)-notin $names){throw "Falta $prefix$name"}}}
  if($names-contains 'index.html'){throw 'El ZIP no conserva la estructura de doble ruta.'}
  if($names|Where-Object{$_-match 'V32[012345]|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\'}){throw 'El ZIP contiene una versión anterior o una entrada no permitida.'}
  foreach($entry in $archive.Entries|Where-Object FullName -match '(^|/)version-web\.json$'){$reader=[IO.StreamReader]::new($entry.Open());try{$json=$reader.ReadToEnd();$metadata=$json|ConvertFrom-Json}finally{$reader.Dispose()};if($metadata.release-ne'V326'){throw 'Versión web incorrecta.'}}
