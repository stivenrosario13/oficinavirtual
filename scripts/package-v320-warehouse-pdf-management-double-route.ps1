...
$publish = Join-Path $root "outputs/_publish_v320_$stamp"
$package = Join-Path $root "outputs/_double_v320_$stamp"
$out = Join-Path $root "outputs/V320_GESTION_SOLICITUDES_PDF_ALMACEN_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V320_GESTION_SOLICITUDES_PDF_ALMACEN_$stamp.zip"
...
try {
  ...
  foreach($prefix in @('','wwwroot/')){foreach($name in @('RegistroAgencias.SqlServer.V320.exe','RegistroAgencias.SqlServer.V320.dll','RegistroAgencias.SqlServer.V320.deps.json','RegistroAgencias.SqlServer.V320.runtimeconfig.json','web.config','wwwroot/index.html','wwwroot/version-web.json','wwwroot/release-guard.js')){if(($prefix+$name)-notin $names){throw "Falta $prefix$name"}}}
  if($names-contains 'index.html'){throw 'El ZIP no conserva la estructura de doble ruta.'}
  if($names|Where-Object{$_-match 'V31[0-9]|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\'}){throw 'El ZIP contiene una versión anterior o una entrada no permitida.'}
  foreach($entry in $archive.Entries|Where-Object FullName -match '(^|/)version-web\.json$'){$reader=[IO.StreamReader]::new($entry.Open());try{$json=$reader.ReadToEnd()}finally{$reader.Dispose()};if($json-notmatch '"release"\s*:\s*"V320"'){throw 'Versión web incorrecta.'}}
