...
$publish = Join-Path $root "outputs/_publish_v323_$stamp"
$package = Join-Path $root "outputs/_double_v323_$stamp"
$out = Join-Path $root "outputs/V323_NAVEGACION_FRANKLIN_FOTOS_UBICACIONES_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V323_NAVEGACION_FRANKLIN_FOTOS_UBICACIONES_$stamp.zip"
...
try {
  ...
  foreach($prefix in @('','wwwroot/')){foreach($name in @('RegistroAgencias.SqlServer.V323.exe','RegistroAgencias.SqlServer.V323.dll','RegistroAgencias.SqlServer.V323.deps.json','RegistroAgencias.SqlServer.V323.runtimeconfig.json','web.config','wwwroot/index.html','wwwroot/version-web.json','wwwroot/release-guard.js','Data/agency-locations-sonadora-20260907.json')){if(($prefix+$name)-notin $names){throw "Falta $prefix$name"}}}
  if($names-contains 'index.html'){throw 'El ZIP no conserva la estructura de doble ruta.'}
  if($names|Where-Object{$_-match 'V32[012]|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\'}){throw 'El ZIP contiene una versión anterior o una entrada no permitida.'}
  foreach($entry in $archive.Entries|Where-Object FullName -match '(^|/)version-web\.json$'){$reader=[IO.StreamReader]::new($entry.Open());try{$json=$reader.ReadToEnd();$metadata=$json|ConvertFrom-Json}finally{$reader.Dispose()};if($metadata.release-ne'V323'){throw 'Versión web incorrecta.'}}
