...
$publish = Join-Path $root "outputs/_publish_v319_$stamp"
$package = Join-Path $root "outputs/_double_v319_$stamp"
$out = Join-Path $root "outputs/V319_ALMACEN_PDF_INSTITUCIONAL_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V319_ALMACEN_PDF_INSTITUCIONAL_$stamp.zip"
...
# Estructura de actualización compatible con Monster: raíz publicable y copia
# completa bajo wwwroot, sin datos, configuración ni archivos de versiones previas.
...
try {
  ...
  foreach ($prefix in @('','wwwroot/')) {
    foreach ($name in @(
      'RegistroAgencias.SqlServer.V319.exe',
      'RegistroAgencias.SqlServer.V319.dll',
      'RegistroAgencias.SqlServer.V319.deps.json',
      'RegistroAgencias.SqlServer.V319.runtimeconfig.json',
      'web.config',
      'wwwroot/index.html',
  ...
  if ($names | Where-Object { $_ -match 'V31[0-8]|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\' }) {
  ...
  foreach ($entry in $archive.Entries | Where-Object FullName -match '(^|/)version-web\.json$') {
    ...
    if ($versionJson -notmatch '"release"\s*:\s*"V319"') { throw 'Versión web incorrecta.' }
  }
