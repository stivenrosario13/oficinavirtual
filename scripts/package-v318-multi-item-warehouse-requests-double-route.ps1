...
$publish = Join-Path $root "outputs/_publish_v318_$stamp"
$package = Join-Path $root "outputs/_double_v318_$stamp"
$out = Join-Path $root "outputs/V318_SOLICITUDES_MULTIPLES_ALMACEN_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V318_SOLICITUDES_MULTIPLES_ALMACEN_$stamp.zip"
...
# Conserva el formato de doble ruta requerido por Monster: publicación normal
# y una segunda copia completa bajo wwwroot.
...
try {
  ...
  foreach ($prefix in @('','wwwroot/')) {
    foreach ($name in @(
      'RegistroAgencias.SqlServer.V318.exe',
      'RegistroAgencias.SqlServer.V318.dll',
      'RegistroAgencias.SqlServer.V318.deps.json',
      'RegistroAgencias.SqlServer.V318.runtimeconfig.json',
      'web.config',
      'wwwroot/index.html',
  ...
  if ($names | Where-Object { $_ -match 'V317|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\' }) {
  ...
  foreach ($entry in $archive.Entries | Where-Object FullName -match '(^|/)version-web\.json$') {
    ...
    if ($versionJson -notmatch '"release"\s*:\s*"V318"') { throw 'Versión web incorrecta.' }
  }
