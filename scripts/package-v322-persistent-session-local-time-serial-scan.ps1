...
$publish = Join-Path $root "outputs/_publish_v322_$stamp"
$package = Join-Path $root "outputs/_double_v322_$stamp"
$out = Join-Path $root "outputs/V322_SESION_HORA_EVIDENCIAS_SERIAL_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V322_SESION_HORA_EVIDENCIAS_SERIAL_$stamp.zip"
...
try {
  ...
  foreach($prefix in @('','wwwroot/')){foreach($name in @('RegistroAgencias.SqlServer.V322.exe','RegistroAgencias.SqlServer.V322.dll','RegistroAgencias.SqlServer.V322.deps.json','RegistroAgencias.SqlServer.V322.runtimeconfig.json','web.config','wwwroot/index.html','wwwroot/version-web.json','wwwroot/release-guard.js')){if(($prefix+$name)-notin $names){throw "Falta $prefix$name"}}}
  if($names-contains 'index.html'){throw 'El ZIP no conserva la estructura de doble ruta.'}
  if($names|Where-Object{$_-match 'V32[01]|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\'}){throw 'El ZIP contiene una versión anterior o una entrada no permitida.'}
  foreach($entry in $archive.Entries|Where-Object FullName -match '(^|/)version-web\.json$'){$reader=[IO.StreamReader]::new($entry.Open());try{$json=$reader.ReadToEnd()}finally{$reader.Dispose()};if($json-notmatch '"release"\s*:\s*"V322"'){throw 'Versión web incorrecta.'}}
