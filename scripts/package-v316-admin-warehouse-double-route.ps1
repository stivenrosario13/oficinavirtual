...
$publish = Join-Path $root "outputs/_publish_v316_$stamp"
$package = Join-Path $root "outputs/_double_v316_$stamp"
$out = Join-Path $root "outputs/V316_SOLICITUDES_ALMACEN_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V316_SOLICITUDES_ALMACEN_$stamp.zip"
...
# Formato doble ruta compatible con el paquete entregado para V315:
# publicación normal y una segunda copia completa dentro de wwwroot.
Get-ChildItem $publish -Force | Where-Object Name -ne 'web.config' |
  Copy-Item -Destination $package -Recurse
$second = Join-Path $package 'wwwroot'
Get-ChildItem $publish -Force | Where-Object Name -ne 'web.config' |
  Copy-Item -Destination $second -Recurse
...
try {
  ...
  foreach ($prefix in @('','wwwroot/')) {
    foreach ($name in @(
      'RegistroAgencias.SqlServer.V316.exe',
      'RegistroAgencias.SqlServer.V316.dll',
      'RegistroAgencias.SqlServer.V316.deps.json',
      'RegistroAgencias.SqlServer.V316.runtimeconfig.json',
      'web.config',
      'wwwroot/index.html',
  ...
  if ($names -contains 'index.html') { throw 'El ZIP no conserva la estructura de V315.' }
  if ($names | Where-Object { $_ -match 'V315|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\' }) {
    throw 'El ZIP contiene una versión anterior o una entrada no permitida.'
  }
  foreach ($entry in $archive.Entries | Where-Object FullName -match '(^|/)version-web\.json$') {
    $reader = [IO.StreamReader]::new($entry.Open())
    try { $versionJson = $reader.ReadToEnd() } finally { $reader.Dispose() }
    if ($versionJson -notmatch '"release"\s*:\s*"V316"') { throw 'Versión web incorrecta.' }
  }
