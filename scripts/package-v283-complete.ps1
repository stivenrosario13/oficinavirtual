...
$stage=Join-Path $root 'artifacts/package-v283'
$out=Join-Path $root 'outputs/V283_ALMACEN_TALLER_COMPLETO'
if((Test-Path $stage) -or (Test-Path $out)){throw 'La salida V283 ya existe; no se sobrescribirá.'}
...
foreach($destination in @($package,$second)){
 Copy-Item (Join-Path $root 'api/web.config') (Join-Path $destination 'web.config.V283.nuevo')
 Copy-Item (Join-Path $root 'deploy/LEEME_V283_MONSTER.txt') (Join-Path $destination 'LEEME_V283_MONSTER.txt')
}
Copy-Item (Join-Path $root 'deploy/LEEME_V283_MONSTER.txt') $out
Add-Type -AssemblyName System.IO.Compression
Add-Type -AssemblyName System.IO.Compression.FileSystem
$zipPath=Join-Path $out 'ACTUALIZACION_COMPLETA_V283_MONSTER.zip'
...
try{
 ...
 foreach($prefix in @('','wwwroot/')){
  foreach($name in @('RegistroAgencias.SqlServer.V283.exe','RegistroAgencias.SqlServer.V283.dll','RegistroAgencias.SqlServer.V283.deps.json','RegistroAgencias.SqlServer.V283.runtimeconfig.json','web.config.V283.nuevo','wwwroot/index.html','wwwroot/version-web.json','wwwroot/vendor/pdfjs/pdf.mjs','wwwroot/documents/plantilla-conduce-institucional.pdf')){
 ...
 foreach($entry in $zip.Entries | Where-Object {$_.FullName -match '(^|/)index.html$'}){
  ...
  if($html -notmatch 'V283'){throw 'Interfaz con versión incorrecta'}
