...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v266-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V266_TARJETAS_ALMACEN_PRIMERO_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V266_TARJETAS_ALMACEN_PRIMERO.txt"
...
Write-Host "1/4 Compilando la interfaz V266..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V266..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V266_TARJETAS_ALMACEN_PRIMERO.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V266_TARJETAS_ALMACEN_PRIMERO.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V266 listo: $destinationZip" -ForegroundColor Green
