...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v274-forced-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$outputRoot = [System.IO.Path]::GetFullPath((Join-Path $outputsRoot "V274_FORZADO_DOBLE_RUTA"))
$packageRoot = Join-Path $outputRoot "paquete"
$destinationZip = Join-Path $outputRoot "FORZAR_VERSION_V274_DOBLE_RUTA.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V274_MODULO_DOCUMENTAL.txt"
$offlineMarker = Join-Path $projectRoot "deploy\app_offline.v274.nuevo"
...
if (Test-Path -LiteralPath $outputRoot) { throw "Ya existe la salida V274. No se sobrescribirá un paquete existente." }
New-Item -ItemType Directory -Path $publishRoot, $packageRoot -Force | Out-Null

Set-Location $projectRoot
Write-Host "1/5 Compilando la interfaz V274..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/5 Publicando el backend V274..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V274_MODULO_DOCUMENTAL.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V274_MODULO_DOCUMENTAL.txt") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $packageRoot "app_offline.v274.nuevo") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $secondApplicationRoot "app_offline.v274.nuevo") -Force
...
$required = @("RegistroAgencias.SqlServer.V274.exe","web.config","wwwroot/index.html","wwwroot/loto-real-logo-transparent.png","wwwroot/version-web.json","wwwroot/vendor/pdfjs/pdf.mjs","wwwroot/vendor/pdfjs/pdf.worker.mjs","wwwroot/RegistroAgencias.SqlServer.V274.exe","wwwroot/wwwroot/index.html","wwwroot/wwwroot/vendor/pdfjs/pdf.mjs","wwwroot/documents/plantilla-conduce-institucional.pdf","wwwroot/wwwroot/documents/plantilla-conduce-institucional.pdf")
...
Write-Host "Paquete V274 listo: $destinationZip" -ForegroundColor Green
