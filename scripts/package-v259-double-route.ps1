...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v259-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V259_LOGIN_EXACTO_SUPERVISOR_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V259_LOGIN_EXACTO_SUPERVISOR.txt"
...
Write-Host "1/4 Compilando la interfaz V259..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V259..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V259_LOGIN_EXACTO_SUPERVISOR.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V259_LOGIN_EXACTO_SUPERVISOR.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V259 listo: $destinationZip" -ForegroundColor Green
