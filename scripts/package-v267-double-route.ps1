...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v267-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V267_GUARDADO_DASHBOARD_COMPRAS_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V267_GUARDADO_DASHBOARD_COMPRAS.txt"
...
Write-Host "1/4 Compilando la interfaz V267..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V267..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V267_GUARDADO_DASHBOARD_COMPRAS.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V267_GUARDADO_DASHBOARD_COMPRAS.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V267 listo: $destinationZip" -ForegroundColor Green
