...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v271-forced-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$outputRoot = [System.IO.Path]::GetFullPath((Join-Path $outputsRoot "V271_FORZADO_DOBLE_RUTA"))
$packageRoot = Join-Path $outputRoot "paquete"
$destinationZip = Join-Path $outputRoot "FORZAR_VERSION_V271_DOBLE_RUTA.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V271_FIRMA_SELLO_REQUERIMIENTOS.txt"
$offlineMarker = Join-Path $projectRoot "deploy\app_offline.v271.nuevo"
...
if (Test-Path -LiteralPath $outputRoot) { throw "Ya existe la salida V271. No se sobrescribirá un paquete existente." }
New-Item -ItemType Directory -Path $publishRoot, $packageRoot -Force | Out-Null

Set-Location $projectRoot
Write-Host "1/5 Compilando la interfaz V271..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/5 Publicando el backend V271..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V271_FIRMA_SELLO_REQUERIMIENTOS.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V271_FIRMA_SELLO_REQUERIMIENTOS.txt") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $packageRoot "app_offline.v271.nuevo") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $secondApplicationRoot "app_offline.v271.nuevo") -Force
...
Write-Host "Paquete V271 listo: $destinationZip" -ForegroundColor Green
