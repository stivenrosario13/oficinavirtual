...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v268-forced-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$outputRoot = [System.IO.Path]::GetFullPath((Join-Path $outputsRoot "V268_FORZADO_DOBLE_RUTA"))
$packageRoot = Join-Path $outputRoot "paquete"
$destinationZip = Join-Path $outputRoot "FORZAR_VERSION_V268_DOBLE_RUTA.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V268_TALLER_ALMACEN_PDF.txt"
$offlineMarker = Join-Path $projectRoot "deploy\app_offline.v268.nuevo"
...
Write-Host "1/5 Compilando la interfaz V268..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/5 Publicando el backend V268..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V268_TALLER_ALMACEN_PDF.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V268_TALLER_ALMACEN_PDF.txt") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $packageRoot "app_offline.v268.nuevo") -Force
Copy-Item -LiteralPath $offlineMarker -Destination (Join-Path $secondApplicationRoot "app_offline.v268.nuevo") -Force
...
$required = @(
    "RegistroAgencias.SqlServer.V268.exe",
    "web.config",
    "wwwroot/index.html",
...
Write-Host "Paquete V268 listo: $destinationZip" -ForegroundColor Green
