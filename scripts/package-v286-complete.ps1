...
$stage=Join-Path $root 'artifacts/package-v286'
$out=Join-Path $root 'outputs/V286_ALMACEN_TALLER_COMPLETO'
if((Test-Path $stage) -or (Test-Path $out)){throw 'La salida V286 ya existe; no se sobrescribirá.'}
...
foreach($destination in @($package,$second)){
 Copy-Item (Join-Path $root 'api/web.config') (Join-Path $destination 'web.config.V286.nuevo')
 Copy-Item (Join-Path $root 'deploy/LEEME_V286_MONSTER.txt') (Join-Path $destination 'LEEME_V286_MONSTER.txt')
}
Copy-Item (Join-Path $root 'deploy/LEEME_V286_MONSTER.txt') $out
Add-Type -AssemblyName System.IO.Compression
Add-Type -AssemblyName System.IO.Compression.FileSystem
$zipPath=Join-Path $out 'ACTUALIZACION_COMPLETA_V286_MONSTER.zip'
...
try{
 ...
 foreach($prefix in @('','wwwroot/')){
  foreach($name in @('RegistroAgencias.SqlServer.V286.exe','RegistroAgencias.SqlServer.V286.dll','RegistroAgencias.SqlServer.V286.deps.json','RegistroAgencias.SqlServer.V286.runtimeconfig.json','web.config.V286.nuevo','wwwroot/index.html','wwwroot/version-web.json','wwwroot/vendor/pdfjs/pdf.mjs','wwwroot/documents/plantilla-conduce-institucional.pdf')){
 ...
 foreach($entry in $zip.Entries | Where-Object {$_.FullName -match '(^|/)index.html$'}){
  ...
  if($html -notmatch 'V286'){throw 'Interfaz con versión incorrecta'}
