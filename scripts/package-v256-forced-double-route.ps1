...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v256-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V256_FORZAR_ACTUALIZACION_DOBLE_RUTA_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V256_FORZADO_DOBLE_RUTA.txt"
...
Write-Host "1/4 Compilando la interfaz V256..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V256..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V256_FORZADO_DOBLE_RUTA.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V256_FORZADO_DOBLE_RUTA.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete forzado listo: $destinationZip" -ForegroundColor Green
