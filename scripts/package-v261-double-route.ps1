...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v261-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V261_FLUJO_ALMACEN_TALLER_MOVIL_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V261_FLUJO_ALMACEN_TALLER_MOVIL.txt"
...
Write-Host "1/4 Compilando la interfaz V261..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V261..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V261_FLUJO_ALMACEN_TALLER_MOVIL.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V261_FLUJO_ALMACEN_TALLER_MOVIL.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V261 listo: $destinationZip" -ForegroundColor Green
