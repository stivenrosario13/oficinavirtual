...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v263-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V263_MIS_TICKETS_ALMACEN_TALLER_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V263_MIS_TICKETS_ALMACEN_TALLER.txt"
...
Write-Host "1/4 Compilando la interfaz V263..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V263..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V263_MIS_TICKETS_ALMACEN_TALLER.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V263_MIS_TICKETS_ALMACEN_TALLER.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V263 listo: $destinationZip" -ForegroundColor Green
