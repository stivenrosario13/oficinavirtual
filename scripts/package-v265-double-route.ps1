...
$stageRoot = [System.IO.Path]::GetFullPath((Join-Path $artifactRoot ".package-v265-double-route"))
$publishRoot = Join-Path $stageRoot "publish"
$packageRoot = Join-Path $stageRoot "package"
$destinationZip = Join-Path $artifactRoot "V265_VISTA_LIMPIA_ALMACEN_TALLER_MONSTER.zip"
$instructions = Join-Path $projectRoot "deploy\LEEME_V265_VISTA_LIMPIA_ALMACEN_TALLER.txt"
...
Write-Host "1/4 Compilando la interfaz V265..." -ForegroundColor Cyan
& npm.cmd run build
if ($LASTEXITCODE -ne 0) { throw "La compilación del frontend falló." }

Write-Host "2/4 Publicando el backend V265..." -ForegroundColor Cyan
...
Copy-Item -LiteralPath $instructions -Destination (Join-Path $packageRoot "LEEME_V265_VISTA_LIMPIA_ALMACEN_TALLER.txt") -Force
Copy-Item -LiteralPath $instructions -Destination (Join-Path $secondApplicationRoot "LEEME_V265_VISTA_LIMPIA_ALMACEN_TALLER.txt") -Force
...
Remove-Item -LiteralPath $stageRoot -Recurse -Force
Write-Host "Paquete V265 listo: $destinationZip" -ForegroundColor Green
