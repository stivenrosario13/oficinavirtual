...
$publish = Join-Path $root "outputs/_publish_v321_$stamp"
$package = Join-Path $root "outputs/_double_v321_$stamp"
$out = Join-Path $root "outputs/V321_CHAT_PUSH_ROLES_$stamp"
$zipPath = Join-Path $out "ACTUALIZACION_V321_CHAT_PUSH_ROLES_$stamp.zip"
...
try {
  ...
  foreach($prefix in @('','wwwroot/')){foreach($name in @('RegistroAgencias.SqlServer.V321.exe','RegistroAgencias.SqlServer.V321.dll','RegistroAgencias.SqlServer.V321.deps.json','RegistroAgencias.SqlServer.V321.runtimeconfig.json','web.config','wwwroot/index.html','wwwroot/version-web.json','wwwroot/release-guard.js')){if(($prefix+$name)-notin $names){throw "Falta $prefix$name"}}}
  if($names-contains 'index.html'){throw 'El ZIP no conserva la estructura de doble ruta.'}
  if($names|Where-Object{$_-match 'V32[0]|(^|/)(\.env[^/]*|appsettings[^/]*|app_offline\.htm)$|(^|/)(App_Data|uploads|preview|logs|node_modules)/|\\'}){throw 'El ZIP contiene una versión anterior o una entrada no permitida.'}
  foreach($entry in $archive.Entries|Where-Object FullName -match '(^|/)version-web\.json$'){$reader=[IO.StreamReader]::new($entry.Open());try{$json=$reader.ReadToEnd()}finally{$reader.Dispose()};if($json-notmatch '"release"\s*:\s*"V321"'){throw 'Versión web incorrecta.'}}
