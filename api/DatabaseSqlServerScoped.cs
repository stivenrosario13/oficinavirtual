...
sealed partial class Database
{
    ...
    public async Task<TicketActionResult> CreateMaintenanceMovement(MaintenanceMovementCreate body,string userId,string role,string? supportTeam,string actorName,CancellationToken ct)
    {
        ...
        var movementType=body.MovementType!.Trim().ToUpperInvariant();var entersWorkshop=requestedArea=="WORKSHOP"||movementType=="TRANSFER_TO_WORKSHOP";var equipment=body.EquipmentType!.Trim();
        ...
        if(warehouse&&movementType is ("EXIT" or "NEW_DELIVERY"))
        {
