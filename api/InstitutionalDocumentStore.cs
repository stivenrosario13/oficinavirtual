...
partial class Database
{
    ...
    public async Task<string?> InstitutionalChange(long id,DocumentActor a,string action,InstitutionalAction b,CancellationToken ct)
    {
        ...
        else
        {
            ...
            if(action=="approve") {if(d.Channel!="requirements"||!a.Manager||d.Status!="received")return "Almacén debe marcar primero el requerimiento como recibido antes de autorizarlo.";d.Status="approved";d.DecisionBy=a.Name+" · "+a.Login;d.DecisionUtc=DateTime.UtcNow;d.CorrectionNote=b.Note;}
            ...
            else if(action is "correction" or "reject") {if(d.Channel=="requirements"&&d.Status!="received")return "Almacén debe marcar primero el requerimiento como recibido antes de decidir.";if(d.Status is not ("pending" or "correction_requested" or "received"))return "La comunicación ya tiene una decisión.";d.Status=action=="reject"?"rejected":"correction_requested";d.CorrectionNote=b.Note;d.DecisionBy=a.Name+" · "+a.Login;d.DecisionUtc=DateTime.UtcNow;}
